...
> **⚠️ 상태: 폐기됨(ABANDONED).** 이 플러그인은 신뢰성 있게 동작하지 않으며 더 이상 유지보수하지 않습니다. herdr 플러그인 API의 근본적 제약 때문에 "제대로 동작"에 도달할 수 없습니다(아래 실패 로그 참고). 기록용으로만 남깁니다.

## 실패 로그 (사후 기록)

**목표:** tmux `synchronize-panes` 재현 — 한 패널에 타이핑하면 같은 탭의 다른 패널에 실시간 미러링.

**왜 안 되는가:** herdr 플러그인/소켓 API(0.7.4)에는 **키 입력 훅도, 출력 변경 이벤트도 없습니다.** tmux는 키를 원천에서 복제하지만 herdr는 불가능합니다. 플러그인이 할 수 있는 건 **화면에 보이는 텍스트를 폴링·비교해 다시 보내는 것**뿐 — 실제 입력 캡처가 아니라 휴리스틱입니다.

**이 휴리스틱이 처리하지 못하는 것 (실제로 시도·확인함):**

- **제어키**(Ctrl+C/L/D, 화살표, Esc, Tab): 입력 줄에 흔적을 안 남겨 감지 불가. 우회책은 키마다 바인딩 하나씩 — 번거롭고, 자주 쓰는 키를 하이재킹하면 누를 때마다 Node 프로세스가 뜸.
- **셸 자동완성**(fish/zsh 회색 힌트): ANSI 색을 벗기면 타이핑한 텍스트와 구분 불가 → 실제 입력처럼 미러링되어 엉뚱한 명령이 다른 패널에 들어감.
- **에코 없는 입력**(비밀번호), **멀티라인 명령**, 완성 메뉴, 대화형 앱(vim/less/REPL)은 미러링 안 됨.
- **프롬프트 인식**은 추측이라 특이한 프롬프트는 정규식을 손으로 써야 함.
- **SSH 중첩 tmux:** `ctrl+b` prefix는 브로드캐스트되지만, 그 *다음* 누르는 tmux 명령키는 tmux가 가로채 에코되지 않아 미러링 불가.

**herdr 운영상 문제도 겹침:**

- startup 훅은 install/link/reload에서 안 뜨고 세션 복원 때만 실행됨.
- 로컬 링크는 물론 설치된 플러그인도 live handoff 후 레지스트리에서 사라짐.
- 키바인딩 자동 등록 수단이 없어 사용자가 `config.toml`을 직접 편집해야 함.

**결론:** 출력 감시 방식은 정확한 입력 동기화에 막다른 길입니다. 신뢰성 있는 다중 패널 입력은 herdr 자체에 키 입력 훅/브로드캐스트 기능이 생기거나(업스트림), tmux를 쓰는 수밖에 없습니다. **프로젝트 폐기.**

아래 문서는 (부분 동작하던) 당시 구현에 대한 참고용 기록입니다.
